...
$(document).ready(function () {
    ...
    $(document).on("click", "#sendToEReaderBtn", function (e) {
        ...
        var isInFloatingModal =
            $("#bookDetailsModal").length > 0 &&
            ($("#bookDetailsModal").hasClass("in") ||
                $("#bookDetailsModal").is(":visible"));
    ...
    $("#sendSelectedBtn").click(function () {
        ...
        if (selectedEmails.length === 0) {
            alert(
                $("#emailSelectModal").data("no-email-message") ||
        ...
        $.ajax({
            ...
            error: function () {
                alert(
                    $("#emailSelectModal").data("error-message") ||
                        "Error sending email"
                );
            },
        });
    ...
    $('input[name="selected_emails"]').change(function () {
        ...
        var checkedCheckboxes = $(
            'input[name="selected_emails"]:checked'
        ).length;
        $("#selectAllEmails").prop(
            "checked",
